...
public class MemberController {
    ...
    public void doJoin(Rq rq) {
        ...
        if(username.trim().isBlank()){
            rq.replace("로그인 아이디를 입력해 주세요.", "/usr/member/join");
            return;
        }
        String password = rq.getParam("password", "");

        if(password.trim().isBlank()){
            rq.replace("비밀번호를 입력해 주세요.", "/usr/member/join");
            return;
        }
        ...
        if(name.trim().isBlank()){
            rq.replace("이름을 입력해 주세요.", "/usr/member/join");
            return;
        }
        ...
        rq.replace("%s 님 환영합니다.".formatted(username), "/");
    ...
    public void doLogin(Rq rq) {
        ...
        if (username.isBlank()) {
            rq.replace("아이디를 입력해 주세요.", "/usr/member/login");
            return;
        }

        if (password.isBlank()) {
            rq.replace("비밀번호를 입력해 주세요.", "/usr/member/login");
            return;
        }
        ...
        if (member == null) {
            rq.replace("존재하지 않는 회원입니다.", "/usr/member/login");
            return;
        }
        ...
        rq.replace("%s 님 로그인이 완료되었습니다.".formatted(username), "/");
    }

    public void doLogout(Rq rq) {
        rq.removeSessionAttr("loggedInUser");
